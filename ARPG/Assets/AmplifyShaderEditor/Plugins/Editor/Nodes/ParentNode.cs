...
namespace AmplifyShaderEditor
{
	...
	public class ParentNode : ScriptableObject, ISerializationCallbackReceiver
	{
		...
		protected void ChangeSize()
		{
			...
			m_position.height += UIUtils.CurrentHeaderHeight + m_extraHeaderHeight + Constants.INPUT_PORT_DELTA_Y + m_extraSize.y;
		...
		public virtual void OnInputPortDisconnected( int portId ) { MarkForPreviewUpdate(); GetInputPortByUniqueId( portId ).UpdatedPreview = false; }
		...
		public virtual void DrawPreview( DrawInfo drawInfo, Rect rect, bool forceNoAlpha = false )
		{
			if ( !m_drawPreview )
				return;
			...
			if( m_previewId == -1 )
			{
				Texture checkers = AssetDatabase.LoadAssetAtPath<Texture>( AssetDatabase.GUIDToAssetPath( "750b1bd7ba8bd28489650de6d0a95cc5" ) );
				EditorGUI.DrawPreviewTexture( rect, checkers );
				return;
			}
			...
			if ( PreviewTexture == null )
			{
				if ( m_outputPorts.Count > 0 )
					m_outputPorts[ 0 ].GeneratePortPreview( 0, m_previewId );
				MarkForPreviewUpdate();
			}
			else
			{
				...
				//else if ( ( m_outputPorts.Count > 0 && ( m_outputPorts[ 0 ].DataType == WirePortDataType.FLOAT4 || m_outputPorts[ 0 ].DataType == WirePortDataType.COLOR ) ) )
				//	EditorGUI.DrawTextureTransparent( rect, PreviewTexture );
				//else
					EditorGUI.DrawPreviewTexture( rect, PreviewTexture );
			}
		}
		...
		private void CalculatedPreviewArea( DrawInfo drawInfo )
		{
			...
			if ( m_canExpand && (m_showPreview || m_globalShowPreview ) )
			{
				...
				m_previewRect.size = new Vector2( previewSize, previewSize ) * drawInfo.InvertedZoom;

				if ( autoLocation == PreviewLocation.BottomCenter )
				{
					m_previewRect.position += new Vector2( scaledMarginAround, scaledMarginAround );
				}
				else if ( autoLocation == PreviewLocation.TopCenter )
				{
					m_previewRect.position += new Vector2( scaledMarginAround, 0 );
				}
				else
				{
			...
			if ( ( m_showPreview || m_globalShowPreview ) )
		...
		public virtual void SetPreviewInputs()
		{
			for ( int i = 0; i < m_inputPorts.Count; i++ )
			{
				if ( !string.IsNullOrEmpty( m_inputPorts[ i ].PreviewSamplerName ) && !m_inputPorts[ i ].IsConnected )
				{
					m_inputPorts[ i ].SetPreviewTexture();
				}
			}
		}

		public void RefreshNodePort(int port)
		{
			if ( m_previewId == -1 || string.IsNullOrEmpty( m_inputPorts[ port ].PreviewSamplerName ) )
			{
				m_inputPorts[ port ].UpdatedPreview = true;
				return;
			}
			
			m_inputPorts[ port ].GeneratePortPreview();
		}

		public void RefreshAllOutputPreviews()
		{
			for ( int i = 0; i < m_outputPorts.Count; i++ )
			{
				m_outputPorts[ i ].GeneratePortPreview( i, m_previewId );
			}
			AfterPreviewRefresh();
		}
		...
		void DrawInternalData()
		{
			for ( int i = 0; i < m_inputPorts.Count; i++ )
			{
				if ( !m_inputPorts[ i ].IsConnected && m_inputPorts[ i ].Visible )
				{
					...
					{
						MarkForPreviewUpdate();
						m_inputPorts[ i ].UpdatedPreview = false;
					}
				}
			}
		}
