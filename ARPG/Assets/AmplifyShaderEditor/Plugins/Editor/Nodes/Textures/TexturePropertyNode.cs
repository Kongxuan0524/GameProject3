...
namespace AmplifyShaderEditor
{
	...
	public class TexturePropertyNode : PropertyNode
	{
		...
		public override void SetPreviewInputs()
		{
			...
			UIUtils.CurrentWindow.PreviewMaterial.SetFloat( "_NotLinear", m_linearTexture ? 0 : 1 );
			UIUtils.CurrentWindow.PreviewMaterial.SetTexture( "_Sampler", Value );
		}
		...
		protected void DrawTexturePicker( DrawInfo drawInfo )
		{
			...
			smallButton.height = 12 * drawInfo.InvertedZoom;
			...
			if ( m_materialMode )
			{
				...
				{
					GUI.Box( newRect, "", "ObjectFieldThumb" );
					...
					GUI.Button( smallButton, "Select", UIUtils.CustomStyle(CustomStyle.SamplerButton) );
					if( UIUtils.CurrentWindow.CurrentGraph.LodLevel <= ParentGraph.NodeLOD.LOD2 )
						GUI.Label( newRect, "None (" + m_currentType.ToString() + ")", "ObjectFieldThumbOverlay" );
				...
				else
				{
					...
					m_drawPreview = false;
					GUI.Box( newRect, string.Empty, UIUtils.CustomStyle( CustomStyle.SamplerFrame ) );
					UIUtils.CustomStyle( CustomStyle.SamplerButton ).fontSize = ( int )Mathf.Round( 9 * drawInfo.InvertedZoom );
					GUI.Button( smallButton, "Select", UIUtils.CustomStyle( CustomStyle.SamplerButton ) );
				}
			}
