...
namespace AmplifyShaderEditor
{
	...
	public sealed class SamplerNode : TexturePropertyNode
	{
		...
		protected override void CommonInit( int uniqueId )
		{
			...
			AddInputPort( WirePortDataType.FLOAT, false, "Normal" );
		...
		//	m_useCustomPrefix = true;
			...
			//m_hasPreview = false;
			m_uvPort.PreviewSamplerName = "_UVs";
			m_normalPort.PreviewSamplerName = "_NormalScale";
		}
		...
		public override void SetPreviewInputs()
		{
			...
			{
				UIUtils.CurrentWindow.PreviewMaterial.SetFloat( "_NotLinear", m_referenceSampler.TextureProperty.IsLinearTexture ? 0 : 1 );
				UIUtils.CurrentWindow.PreviewMaterial.SetTexture( m_cachedSamplerId, m_referenceSampler.TextureProperty.Value );
			}
			else
			{
				UIUtils.CurrentWindow.PreviewMaterial.SetFloat( "_NotLinear", TextureProperty.IsLinearTexture ? 0 : 1 );
				UIUtils.CurrentWindow.PreviewMaterial.SetTexture( m_cachedSamplerId, TextureProperty.Value );
			}

			UIUtils.CurrentWindow.PreviewMaterial.SetFloat( m_cachedUvsId, (m_uvPort.IsConnected ? 1 : 0) );
		}
		...
		private void DrawTexturePropertyPreview( DrawInfo drawInfo, bool instance )
		{
			...
			{
				m_drawPreview = true;
				DrawPreview( drawInfo, m_previewRect, true );
				m_drawPreview = false;
				GUI.Box( newPos, string.Empty, UIUtils.CustomStyle( CustomStyle.SamplerFrame ) );
				UIUtils.CustomStyle( CustomStyle.SamplerButton ).fontSize = ( int )Mathf.Round( 9 * drawInfo.InvertedZoom );
