...
namespace AmplifyShaderEditor
{
	...
	public sealed class StandardSurfaceOutputNode : MasterNode, ISerializationCallbackReceiver
	{
		...
		public void DrawRenderingOptions( GUIStyle toolbarstyle )
		{
			...
			UIUtils.CurrentWindow.ExpandedRenderingOptions = GUILayout.Toggle( UIUtils.CurrentWindow.ExpandedRenderingOptions, RenderingOptionsStr, m_inspectorFoldoutStyle );
			if ( EditorGUI.EndChangeCheck() )
			{
				EditorPrefs.SetBool( "ExpandedRenderingOptions", UIUtils.CurrentWindow.ExpandedRenderingOptions );
			}
			EditorGUILayout.EndHorizontal();
			if ( UIUtils.CurrentWindow.ExpandedRenderingOptions )
			{
		...
		public void DrawRenderingPlatforms( GUIStyle toolbarstyle )
		{
			...
			GUI.color = cachedColor;
			EditorGUI.BeginChangeCheck();
			UIUtils.CurrentWindow.ExpandedRenderingPlatforms = GUILayout.Toggle( UIUtils.CurrentWindow.ExpandedRenderingPlatforms, RenderingPlatformsStr, m_inspectorFoldoutStyle );
			if ( EditorGUI.EndChangeCheck() )
			{
				EditorPrefs.SetBool( "ExpandedRenderingPlatforms", UIUtils.CurrentWindow.ExpandedRenderingPlatforms );
			}
			EditorGUILayout.EndHorizontal();
			if ( UIUtils.CurrentWindow.ExpandedRenderingPlatforms )
			{
				cachedColor = GUI.color;
				GUI.color = new Color( cachedColor.r, cachedColor.g, cachedColor.b, ( EditorGUIUtility.isProSkin ? 0.5f : 0.25f ) );
				EditorGUILayout.BeginVertical( "TE NodeBackground" );
		...
		public override void DrawProperties()
		{
			...
			if ( m_inspectorPopdropdownStyle == null || EditorGUIUtility.isProSkin != m_usingProSkin )
			{
				...
				//m_inspectorPopdropdownStyle.clipping = TextClipping.Clip;
			}
			...
			EditorGUILayout.BeginVertical();
			{
				...
				EditorGUI.BeginChangeCheck();
				UIUtils.CurrentWindow.ExpandedMain = GUILayout.Toggle( UIUtils.CurrentWindow.ExpandedMain, " General", "foldout" );
				if ( EditorGUI.EndChangeCheck() )
				{
				...
				GUI.color = m_cachedColor;
				EditorGUI.BeginChangeCheck();
				UIUtils.CurrentWindow.ExpandedBlend = GUILayout.Toggle( UIUtils.CurrentWindow.ExpandedBlend, AlphaModeContent, "foldout", GUILayout.ExpandWidth( true ) );
				if ( EditorGUI.EndChangeCheck() )
				{
				...
				EditorGUI.BeginChangeCheck();
				m_alphaMode = ( AlphaMode ) EditorGUILayout.Popup( "", ( int ) m_alphaMode, FadeModeOptions, m_inspectorPopdropdownStyle, GUILayout.Width( boxSize ), GUILayout.Height( 19 ) );
				if ( EditorGUI.EndChangeCheck() )
				{
				...
				DrawRenderingOptions( m_inspectorToolbarStyle );
		...
		public void AddLocalVarInstructions( bool normalIsConnected )
		{
			...
			for ( int i = 0; i < count; i++ )
			{
				...
				if ( isVertex )
				{
					...
					if ( UIUtils.CurrentDataCollector.DirtyVertexVariables )
					{
						UIUtils.CurrentDataCollector.AddVertexInstruction( UIUtils.CurrentDataCollector.VertexLocalVariables, sortedList[ i ].UniqueId, false );
						UIUtils.CurrentDataCollector.ClearVertexLocalVariables();
					}
		...
		public override Shader Execute( string pathname, bool isFullPath )
		{
			...
			if ( m_inputPorts[ m_inputPorts.Count - 1 ].IsConnected )
			{
				...
				UIUtils.CurrentDataCollector.AddInstructions( "\t\t\to.Normal = float3(0,0,1);\n" );
			...
			OpenShaderBody( ref ShaderBody, m_shaderName );
			{
				...
				OpenSubShaderBody( ref ShaderBody );
				{
					...
					if ( m_outlineHelper.EnableOutline )
					{
						AddMultilineBody( ref ShaderBody, m_outlineHelper.OutlineFunctionBody( isInstancedShader, m_customShadowCaster , UIUtils.RemoveInvalidCharacters( ShaderName )) );
					}
						...
						OpenCGProgram( ref ShaderBody );
					{
						...
						if ( UIUtils.CurrentDataCollector.UsingHigherSizeTexcoords )
						{
							...
							ShaderBody += "\t\t#define TRANSFORM_TEX(tex,name) float4(tex.xy * name##_ST.xy + name##_ST.zw, 0, 0)\n";
						}
