...
namespace AmplifyShaderEditor
{
	...
	public sealed class ComponentMaskNode : ParentNode
	{
		...
		protected override void CommonInit( int uniqueId )
		{
			...
			m_inputPorts[ 0 ].PreviewSamplerName = "_MaskTex";
		...
		public override void SetPreviewInputs()
		{
			...
			if ( m_outputPorts[ 0 ].DataType >= WirePortDataType.FLOAT )
				mask.x = m_selection[ 0 ] ? 1 : 0;
			if ( m_outputPorts[ 0 ].DataType >= WirePortDataType.FLOAT2 )
				mask.y = m_selection[ 1 ] ? 1 : 0;
			if ( m_outputPorts[ 0 ].DataType >= WirePortDataType.FLOAT3 )
				mask.z = m_selection[ 2 ] ? 1 : 0;
			if ( m_outputPorts[ 0 ].DataType >= WirePortDataType.FLOAT4 )
				mask.w = m_selection[ 3 ] ? 1 : 0;
