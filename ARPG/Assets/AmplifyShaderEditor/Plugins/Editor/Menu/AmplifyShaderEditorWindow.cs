...
public class AmplifyShaderEditorWindow : EditorWindow, ISerializationCallbackReceiver
{
	...
	public void OnEnable()
	{
		...
		m_shortcutManager.RegisterShortcut( KeyCode.C, "Create Commentary", () =>
		{
			// Create commentary
		...
		m_shortcutManager.RegisterShortcut( KeyCode.F, "Focus On Selection", () =>
		{
			OnToolButtonPressed( ToolButtonType.FocusOnSelection );
			ForceRepaint();
		} );
		
		m_shortcutManager.RegisterShortcut( KeyCode.Space, "Open Node Palette", () =>
		 {
			 m_contextPalette.Show( m_currentMousePos2D, m_cameraInfo );
		 } );


		m_shortcutManager.RegisterShortcut( KeyCode.W, "Toggle Debug Mode", () =>
	   {
		...
		m_shortcutManager.RegisterShortcut( KeyCode.P, "Global Preview", () =>
		{
		...
		m_shortcutManager.RegisterShortcut( KeyCode.Delete, "Delete selected nodes", () =>
		{
			DeleteSelectedNodes();
			ForceRepaint();
		} );

		m_shortcutManager.RegisterShortcut( KeyCode.Backspace, "Delete selected nodes", () =>
		{
			DeleteSelectedNodes();
			ForceRepaint();
		} );
	...
	void OnKeyboardDown()
	{
		...
		{
			m_shortcutManager.ActivateShortcut( m_currentEvent.keyCode );
		}
	...
	public void UpdateTime()
	{
		...
		PreviewMaterial.SetFloat( "_EditorTime", ( float ) m_time );
		PreviewMaterial.SetFloat( "_EditorDeltaTime", ( float ) deltaTime );
	}
	...
	{
		//Debug.Log( CurrentGraph.NodePreviewList.Count );

		if ( CurrentGraph.NodePreviewList.Count > 0 )
		{
			if ( CurrentGraph.NodePreviewList[ 0 ] != null )
			{
				ParentNode node = CurrentGraph.NodePreviewList[ 0 ];
				//Debug.Log( node );
				node.SetPreviewInputs();
				if ( node.InputPorts.Count > 0 )
				{
					...
					{
						//Debug.Log( node.GetType().ToString()+" "+ node.InputPorts[ i ].UpdatedPreview );
						if ( !node.InputPorts[ i ].UpdatedPreview )
						{
							portsToUpdate++;
							if ( portsToUpdate == 1 )
								firstToUpdate = i;
						...
						{
							// Set texture to all connected ports
							node.InputPorts[ i ].SetPreviewTextureFromOutput();
						}
					}
					//Debug.Log( node );
					m_currentPort = firstToUpdate;

					// Calculate preview one input port at a time
					node.RefreshNodePort( m_currentPort );
					m_currentPort++;

					if ( portsToUpdate <= 1 )
					{
						CurrentGraph.NodePreviewList.RemoveAt( 0 );
						m_currentPort = 0;
						node.RefreshAllOutputPreviews();
					}
				}
				else
				{
